...
const STRATEGY_VERSION_ID = "portfolio-v14-activity-clock-retrace";
const STRATEGY_VERSION_NAME = "新规则版本：Bonereaper 时钟同步 + 5m 流量回撤挂单";
...
function parseArgs(argv) {
  ...
  opts.cloneMinVisibleDepthUsdc = clampNum(opts.cloneMinVisibleDepthUsdc, 0, 500, 1);
...
function cloneOptsForSlug(opts, slug) {
  ...
  return {
    ...
    cloneFlowConfirmMinDominance: isFiveMinute ? 0.75 : opts.cloneFlowConfirmMinDominance,
    cloneFlowConfirmMinRows: isFiveMinute ? (asset === "ETH" ? 8 : 18) : opts.cloneFlowConfirmMinRows,
    cloneFlowConfirmMinUsdc: isFiveMinute ? (asset === "ETH" ? 24 : 120) : opts.cloneFlowConfirmMinUsdc,
    cloneFlowConfirmMaxAgeSec: isFiveMinute ? 42 : opts.cloneFlowConfirmMaxAgeSec,
    cloneFlowConfirmOpeningWindowSec: isFiveMinute ? 140 : opts.cloneFlowConfirmOpeningWindowSec,
    cloneFlowConfirmOpeningMaxAgeSec: isFiveMinute ? 210 : opts.cloneFlowConfirmOpeningMaxAgeSec,
    cloneFlowConfirmOpeningRowRatio: isFiveMinute ? 0.65 : opts.cloneFlowConfirmOpeningRowRatio,
    cloneFlowConfirmMaxOpposingParentBps: isFiveMinute ? 5 : opts.cloneFlowConfirmMaxOpposingParentBps,
    cloneFlowConfirmMinEdge: isFiveMinute ? -0.006 : opts.cloneFlowConfirmMinEdge,
    cloneMinSignalBps: isFiveMinute ? Math.max(num(opts.cloneMinSignalBps), 3.2) : opts.cloneMinSignalBps,
    cloneMinExpectedEdge: isFiveMinute ? Math.max(num(opts.cloneMinExpectedEdge), 0.024) : opts.cloneMinExpectedEdge,
    cloneMaxAsk: isFiveMinute ? Math.min(num(opts.cloneMaxAsk), 0.94) : opts.cloneMaxAsk,
  };
}
...
function applyAdaptiveConfig(state, clone, nowSec = Math.floor(Date.now() / 1000)) {
  ...
  if ((clone.horizon || slugHorizon(clone.market?.slug)) === "5m") {
    const fiveMinuteBudgetCap = (clone.asset || slugAsset(clone.market?.slug)) === "ETH" ? 16 : 30;
    cfg.parentConfirmOnly = true;
    cfg.allowLearnedProbe = false;
    cfg.allowComboEntry = false;
    cfg.flowConfirmEnabled = true;
    cfg.flowConfirmMinDominance = Math.max(num(cfg.flowConfirmMinDominance, 0.75), 0.75);
    cfg.flowConfirmMinRows = Math.max(num(cfg.flowConfirmMinRows, 0), (clone.asset || "") === "ETH" ? 8 : 18);
    cfg.flowConfirmMinUsdc = Math.max(num(cfg.flowConfirmMinUsdc, 0), (clone.asset || "") === "ETH" ? 24 : 120);
    cfg.flowConfirmMaxAgeSec = Math.max(num(cfg.flowConfirmMaxAgeSec, 0), 42);
    cfg.flowConfirmOpeningWindowSec = Math.max(num(cfg.flowConfirmOpeningWindowSec, 0), 140);
    cfg.flowConfirmOpeningMaxAgeSec = Math.max(num(cfg.flowConfirmOpeningMaxAgeSec, 0), 210);
    cfg.flowConfirmOpeningRowRatio = clamp(num(cfg.flowConfirmOpeningRowRatio, 0.65), 0.3, 1, 0.65);
    cfg.flowConfirmMaxOpposingParentBps = Math.max(num(cfg.flowConfirmMaxOpposingParentBps, 0), 5);
    cfg.flowConfirmMinEdge = Math.min(num(cfg.flowConfirmMinEdge, -0.006), -0.006);
    ...
    cfg.orderUsdc = round(clamp(num(cfg.orderUsdc), POLYMARKET_MIN_ORDER_USDC, (clone.asset || "") === "ETH" ? 1.7 : 2.2), 2);
    cfg.maxClipUsdc = round(Math.min(num(cfg.maxClipUsdc), (clone.asset || "") === "ETH" ? 2.8 : 3.8), 2);
    cfg.minSignalBps = round(Math.max(num(cfg.minSignalBps), 3.2), 3);
    cfg.minExpectedEdge = round(Math.max(num(cfg.minExpectedEdge), 0.024), 4);
    cfg.maxAsk = round(Math.min(num(cfg.maxAsk), 0.94), 3);
    cfg.targetOrderRows = Math.min(num(cfg.targetOrderRows, 0) || 12, (clone.asset || "") === "ETH" ? 10 : 14);
    Object.assign(adjustments, {
    ...
    suggestions.push("5m flow-carry mode: parent-confirmed entries, fresh Bonereaper flow, or opening flow carry with reduced size");
  }
  clone.config = cfg;
  clone.adaptive = {
    enabled,
...
function fillOpenCloneOrders(clone, nowSec) {
  for (const order of clone.orders || []) {
    ...
    if (Number.isFinite(quote?.ask) && quote.ask <= order.limitPrice) {
...
function generateCloneOrders(clone, nowSec) {
  ...
  for (const candidate of selectedCapped) {
    ...
    if (candidate.style === "cross" && depthUsdc > 0 && depthUsdc < cfg.minVisibleDepthUsdc) continue;
...
function placeCloneOrder(clone, spec) {
  ...
  if (Number.isFinite(spec.quoteAsk) && spec.quoteAsk <= order.limitPrice) {
...
function simulateCloneFill(clone, order, nowSec, reason) {
  const quote = clone.orderbook?.[order.direction];
  if (!quote || !Number.isFinite(quote.ask) || quote.ask > order.limitPrice) return null;
  const cfg = clone.config || {};
  ...
  const participation = makerLike ? num(cfg.liquidityParticipation) * 0.45 : num(cfg.liquidityParticipation);
  const levels = (Array.isArray(quote.asks) ? quote.asks : [])
    .map((level) => ({ price: num(level.price), size: num(level.size) }))
    .filter((level) => Number.isFinite(level.price) && Number.isFinite(level.size) && level.price > 0 && level.size > 0 && level.price <= order.limitPrice)
    .sort((a, b) => a.price - b.price);
  if (!levels.length && quote.ask <= order.limitPrice) levels.push({ price: quote.ask, size: remaining / Math.max(0.05, participation) });
  let fillShares = 0;
  let tradeCost = 0;
  let fee = 0;
  const usedLevels = [];
...
function simulateCloneSell(clone, outcome, shares, limitPrice) {
  ...
  if (!levels.length) levels.push({ price: quote.bid, size: shares / Math.max(0.05, participation) });
  let soldShares = 0;
  let grossProceeds = 0;
  let fee = 0;
  const usedLevels = [];
...
function compactState(state, opts) {
  ...
  if (state.clone) {
    state.clone.orders = (state.clone.orders || []).slice(0, 1000);
    state.clone.fills = (state.clone.fills || []).slice(0, 1000);
  }
  state.cloneHistory = (state.cloneHistory || []).slice(0, 1000);
}
...
<body>
  ...
  <script>
    ...
    function render() {
      ...
      els.minTradeRule.textContent = '成交>= ' + esc(ac.minFillUsdc ?? 1) + 'U / 订单>=盘口最小';
      els.adaptiveParams.textContent = '预算 ' + esc(ac.budgetUsdc ?? '-') + ' / 目标笔数 ' + esc(ac.targetOrderRows ?? '-') + ' / 探针 ' + esc(ac.probeOrderUsdc ?? '-') + ' / 单笔 ' + esc(ac.orderUsdc ?? '-') + ' / 最大 ' + esc(ac.maxClipUsdc ?? '-') + ' / 吃单费 ' + esc(ac.takerFeeRate ?? '-') + ' / 退出 ' + esc(ac.exitEnabled ? '开' : '关') + ' / 反向 ' + esc(ac.hedgeEnabled ? '开' : '关');
